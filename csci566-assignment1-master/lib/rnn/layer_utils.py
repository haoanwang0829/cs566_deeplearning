...
class VanillaRNN(object):
    ...
    def step_backward(self, dnext_h, meta):
        ...
        ds = dnext_h * (1 - np.power(next_h, 2)) #dl/ds where s = tanh(...)
    
        dprev_h = np.dot(ds, self.params[self.wh_name].T)
        dWx = np.dot(x.T, ds)
        dWh = np.dot(prev_h.T, ds)
        db = np.sum(ds, axis=0)

        dx = np.dot(ds, self.params[self.wx_name].T)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
    ...
    def backward(self, dh):
        ...
        for t in range(T-1,-1,-1):
            dx_t, dh0, dWx_t, dWh_t, db_t = self.step_backward(dh[:,t,:]+dh0, self.meta[t])
            dWx[:,t,:] = dWx_t
            dWh[:,t,:] = dWh_t
            db[:,t] = db_t
            dx[:, t, :] = dx_t
        self.grads[self.wx_name] = np.sum(dWx, axis=1)
        self.grads[self.wh_name] = np.sum(dWh, axis=1)
        self.grads[self.b_name] = np.sum(db, axis=1)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        self.meta = []
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        #############################################################################
        s = np.dot(x, self.params[self.wx_name]) + np.dot(prev_h, self.params[self.wh_name]) + self.params[self.b_name]
        s_list = [s[:, i*self.h_dim:(i+1)*self.h_dim] for i in range(4)]
        activate_func = [sigmoid]*3 + [np.tanh]
        i, f, o, g = [f(s_) for f, s_ in zip(activate_func, s_list)]
        next_c = np.multiply(f, prev_c) + np.multiply(i, g)
        next_h = np.multiply(o, np.tanh(next_c))
        meta = (x, prev_h, next_h, i, f, o, g, prev_c, next_c)
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x, prev_h, next_h, i, f, o, g, prev_c, next_c = meta
        fs = lambda x: np.multiply(np.multiply(o, np.multiply(1 - np.power(np.tanh(next_c), 2), x)), dnext_h)
        

        dprev_c = np.multiply(dnext_c, f) +  fs(f)
        d_i = np.multiply(dnext_c, g) +      fs(g)
        d_f = np.multiply(dnext_c, prev_c) + fs(prev_c)
        d_g = np.multiply(dnext_c, i) +      fs(i)
        d_o = np.multiply(dnext_h, np.tanh(next_c))

        d_i = np.multiply(d_i, np.multiply(i, (1 - i)))
        d_f = np.multiply(d_f, np.multiply(f, (1 - f)))
        d_o = np.multiply(d_o, np.multiply(o, (1 - o)))
        d_g = np.multiply(d_g, 1 - np.power(g, 2))
        d_concate= np.concatenate((d_i, d_f, d_o, d_g), axis=1)
        
        dWh = np.dot(prev_h.T, d_concate)
        dWx = np.dot(x.T, d_concate)
        db = d_concate.sum(axis=0)

        dx = np.dot(d_concate, self.params[self.wx_name].T)
        dprev_h = np.dot(d_concate, self.params[self.wh_name].T)
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def forward(self, x, h0):
        ...
        prev_h, prev_c = h0, np.zeros(h0.shape)
        for t in range(x.shape[1]):
            prev_h, prev_c, meta = self.step_forward(x[:, t, :], prev_h, prev_c)
            h[:,t,:]= prev_h
            self.meta.append(meta)
        #############################################################################
        #                               END OF YOUR CODE                            #
    ...
    def backward(self, dh):
        ...
        for t in range(T-1, -1, -1):
            dx_t, dh0, dc0, dWx_t, dWh_t, db_t = self.step_backward(dh[:,t,:]+dh0, dc0, self.meta[t])
            dWx[:,t,:] = dWx_t
            dWh[:,t,:] = dWh_t
            db[:,t] = db_t
            dx[:, t, :] = dx_t
...
class word_embedding(object):
    ...
    def forward(self, x):
        ...
        ##############################################################################
        out = np.array([self.params[self.w_name][x[i, :]] for i in range(x.shape[0])])
        self.meta = x
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
    ...
    def backward(self, dout):
        ...
        for i in range(dout.shape[0]):
            np.add.at(self.grads[self.w_name], x[i, :], dout[i, :, :])
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
